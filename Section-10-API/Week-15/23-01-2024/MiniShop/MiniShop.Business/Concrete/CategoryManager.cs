...
namespace MiniShop.Business.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<Response<NoContent>> HardDeleteAsync(int id)
        {
            ...
            if(category == null)
            {
                return Response<NoContent>.Fail("Hiç kategori bulunamadı", 301);
            }
        ...
        public async Task<Response<NoContent>> SoftDeleteAsync(int id)
        {
            ...
            {
                return Response<NoContent>.Fail("Hiç kategori bulunamadı", 404);
            }
        ...
        public async Task<Response<List<CategoryDTO>>> GetIsDeletedCategoriesAsync(bool IsDeleted = false)
        {
            var categoryList = await _repository.GetAllAsync(c => c.IsDeleted == IsDeleted);
            string status = IsDeleted ? "Silinmiş" : "Silinmemiş";
            if (categoryList == null)
            {
                return Response<List<CategoryDTO>>.Fail($"{status} kategori bulunamadı", 301);
            }
            var categoryDtoList = _mapper.Map<List<CategoryDTO>>(categoryList);
            return Response<List<CategoryDTO>>.Success(categoryDtoList, 200);

        }
        public async Task<Response<List<CategoryDTO>>> GetIsActiveCategoriesAsync(bool IsActive=true)
        {
