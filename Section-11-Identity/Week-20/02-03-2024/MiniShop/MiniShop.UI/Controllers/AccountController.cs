    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel loginViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var duration = _userManager.Options.Lockout.DefaultLockoutTimeSpan.TotalSeconds;
                    //await _userManager.SetLockoutEndDateAsync(user, DateTimeOffset.UtcNow.AddSeconds(duration));
                    _notyfService.Error($"Hesabınız {duration} saniye kilitlendi.");
                    return View(loginViewModel);
                ...
                else
                {
                    ...
                    if ( accesFailedCount == 0)
                    {
                        await _userManager.SetLockoutEnabledAsync(user, true);
                        _notyfService.Error($"Hesabınız {attemptCount} kez hatalı giriş yaptığı için kilitlendi.");
                    }else
                    {
                        _notyfService.Information($"Kalan hakkınız {accesFailedCount}");
                        return View(loginViewModel);
                    }
